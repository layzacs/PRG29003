...
namespace prglib {
    ...
    template<class T> void lista<T>::sublista(lista<T> & sub, int pos1, int num) {
        ...
        if (num < 0) {
            ...
            int pos_final = pos1 + num;
            // se a posição passada mais o num for menor que zero, quer dizer q num ultrapassa os valores da lista. setamos ele pra 0.

            while (pos1 > pos_final) {
                auto nodo = obtemNodo(pos1);
                sub.anexa(nodo->dado);
                pos1--;
            }
